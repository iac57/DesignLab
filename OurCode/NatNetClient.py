...
def save_csv(frame_number, rigid_body_data, filename="rigid_body_data.csv"): 
    ...
    with open(filename, mode="a", newline="") as file: 
        ...
        rigid_body_count = len(rigid_body_data.rigid_body_list)
        print(f"Frame {frame_number}:{rigid_body_count} rigid bodies found")

        for rigid_body in rigid_body_data.rigid_body_list: # Extract position and orientation 
            position = rigid_body.pos # (x, y, z) 
            orientation = rigid_body.rot # (w, x, y, z) 
            # Write data for each rigid body 
            writer.writerow([frame_number, rigid_body.id_num, *position, *orientation])
...
class NatNetClient:
    ...
    def __init__( self ):
        ...
        self.local_ip_address = "192.168.1.163"
...
#Unpack Mocap Data Functions
    ...
    def __process_message( self, data : bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA :
            ...
            rigid_body_data = MoCapData.RigidBodyData()
            #rigid_body_data_str=rigid_body_data.get_as_string()
            save_csv(mocap_data.prefix_data.frame_number, rigid_body_data)
